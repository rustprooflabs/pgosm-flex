...
COMMENT ON COLUMN osm.traffic_point.osm_type IS 'Value of the main key associated with traffic details.  If osm_subtype IS NULL then key = "highway".  Otherwise the main key is the value stored in osm_type while osm_subtype has the value for the main key.';
COMMENT ON COLUMN osm.traffic_line.osm_type IS 'Value of the main key associated with traffic details.  If osm_subtype IS NULL then key = "highway".  Otherwise the main key is the value stored in osm_type while osm_subtype has the value for the main key.';
...
COMMENT ON COLUMN osm.traffic_point.osm_subtype IS 'Value of the non-main key(s) associated with traffic details. See osm_type column for the key associated with this value. NULL when the main key = "highway".';
COMMENT ON COLUMN osm.traffic_line.osm_subtype IS 'Value of the non-main key(s) associated with traffic details. See osm_type column for the key associated with this value. NULL when the main key = "highway".';
